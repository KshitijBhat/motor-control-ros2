...
{   
    // counter
    pio_sm_exec_wait_blocking(pio_0, sm0, pio_encode_in(pio_x, 32));
    encoder_count_s = pio_sm_get_blocking(pio_0, sm0);

    pio_sm_exec_wait_blocking(pio_1, sm1, pio_encode_in(pio_x, 32));
    encoder_count_d = pio_sm_get_blocking(pio_1, sm1);
}
...
int main(){
    ...
    uint offset0 = pio_add_program(pio_0, &quadrature_program);
    uint offset1 = pio_add_program(pio_1, &quadrature_program);
    uint sm0 = pio_claim_unused_sm(pio_0, true);
    uint sm1 = pio_claim_unused_sm(pio_1, true);
    quadrature_program_init(pio_0, sm0, offset0, QUADRATURE1_A_PIN, QUADRATURE1_B_PIN);
    quadrature_program_init(pio_1, sm1, offset1, QUADRATURE2_A_PIN, QUADRATURE2_B_PIN);
